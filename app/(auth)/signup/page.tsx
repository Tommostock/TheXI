...
export default function SignupPage() {
  ...
  const inputClass =
    'mt-1 block w-full rounded-lg border border-border bg-bg-primary px-3 py-2.5 text-white placeholder-text-muted focus:border-wc-lime focus:outline-none focus:ring-1 focus:ring-wc-lime'
      ...
      <div className="w-full max-w-sm rounded-2xl border border-border bg-bg-card p-6">
        ...
        {confirmationSent ? (
          <div className="mt-8 rounded-xl border border-wc-teal/30 bg-wc-teal/10 p-4 text-center">
            ...
            <Link
              href="/login"
              className="mt-4 inline-block rounded-lg bg-wc-lime px-4 py-2 text-sm font-bold text-bg-primary transition-opacity hover:opacity-90"
            >
              Go to Login
            </Link>
          </div>
          ...
          <>
            <form onSubmit={handleSubmit} className="mt-8 space-y-4">
              ...
              <button type="submit" disabled={loading}
                className="w-full rounded-lg bg-wc-lime px-4 py-2.5 font-bold text-bg-primary transition-opacity hover:opacity-90 disabled:opacity-50">
